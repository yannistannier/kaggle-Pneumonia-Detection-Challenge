...
path_train_images = "/home/mbenhamd/.kaggle/competitions/rsna-pneumonia-detection-challenge/train_images/"
path_test_images = "/home/mbenhamd/.kaggle/competitions/rsna-pneumonia-detection-challenge/test_images/"


# In[3]:


path_train = "/home/mbenhamd/.kaggle/competitions/rsna-pneumonia-detection-challenge/stage_1_train_labels.csv"
# empty dictionary
pneumonia_locations = {}
# load table
with open(path_train, mode='r') as infile:
    # open reader
...
percentage = 0.9
folder = path_train_images
filenames = os.listdir(folder)
random.shuffle(filenames)
# split into train and validation filenames
n_valid_samples = int((1 - percentage) * len(filenames))
...
def get_tags(in_path):
    ...
    return pd.Series(tag_dict).values
...
class generator(keras.utils.Sequence):
    ...
    def __load__(self, filename):
        ...
        # load tags 
        #tag = get_tags(os.path.join(self.folder, filename))
        # create empty mask
        ...
        # Image augmentation        
            ...
            msk = det.augment_image(msk.astype(np.uint8))
...
def create_network(input_size, channels, n_blocks=N_BLOCKS, depth=DEPTH):
    # input image and meta_data
    input_img = keras.Input(shape=(input_size, input_size, 1))        
    x = keras.layers.Conv2D(channels,3, padding='same', use_bias=False)(input_img)
    ...
    #merge = keras.layers.concatenate([x, y])
    #final = keras.layers.Dense(LSTM_UNITS, activation='sigmoid')(merge)  
    outputs = keras.layers.UpSampling2D(2**depth)(x)        
    model = keras.Model(inputs=input_img, outputs=outputs)
    # summarize layers
    #pd.DataFrame(model.summary()).to_csv('model_detail.csv')
    #plot graph
    #keras.utils.plot_model(model, to_file='residual_cnn.png')
    #print(model.summary())
    return model
...
filepath= "weights-improvement-{epoch:02d}-{val_mean_iou:.4f}.h5"
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_mean_iou', verbose=1, save_best_only=False, save_weights_only=False, mode='max')
history = model.fit_generator(train_gen, validation_data=valid_gen, callbacks=[learning_rate,checkpoint], epochs=EPOCH, shuffle=True)
